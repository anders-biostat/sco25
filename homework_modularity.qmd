...
that tells us how many edges there are between vertices in cluster $c$ and $c'$. Note that, if defined this way, the diagonal elements $B_{cc}$ count each edge within cluster $c$ twice.
...
You change a given cluster assignment $\mathbf{c}^{(0)}$ into another one, $\mathbf{c}^{(1)}$ by moving a single vertex $v$ from cluster $c_v^{(0)}$ to a new cluster  $c_v^{(1)}$. All other vertices stay in their clusters, i.e., $c_{v'}^{(0)}=c_{v'}^{(1)}$ for all $v'\neq v$.

How can you calculate the change in modularity score, $M_G(\mathbf{c}')-M_G(\mathbf{c})$ without recalculating everything? How do you update $B$?
